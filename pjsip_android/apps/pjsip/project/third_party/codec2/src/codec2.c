                   ...
                   const unsigned char * bits)
{
    ...
    Wo_index = unpack_codec2(bits, &nbit, WO_BITS);
    for(i=0; i<LPC_ORD; i++) {
	lsp_indexes[i] = unpack_codec2(bits, &nbit, lsp_bits(i));
    }
    energy_index = unpack_codec2(bits, &nbit, E_BITS);
    voiced1 = unpack_codec2(bits, &nbit, 1);
    voiced2 = unpack_codec2(bits, &nbit, 1);
    assert(nbit == CODEC2_BITS_PER_FRAME);
